...
public class TouristStatistics {
	...
	*/
	...
	public static void runCorssMonthAnalysis(String gt_ser_file_from, String gt_ser_file_to, String dir) throws Exception {
		new File(dir.replaceAll("_", "-")).mkdirs(); // output dir
		double[][]  cross = corssMonthAnalysis(gt_ser_file_from,gt_ser_file_to);
		
		//System.out.println(gt_ser_file_from);
		//System.out.println(gt_ser_file_to);
		//System.out.println(dir);
		
		
		String fname = getName(gt_ser_file_from,false)+"_"+getName(gt_ser_file_to,false);
		
		String[] labels = new String[GTExtractor.PROFILES.length];
		for(int i=0; i<labels.length;i++)
			labels[i] = GTExtractor.PROFILES[i].substring(0,2)+".";
		
		//RPlotter.drawCorr(dir+"/cross_month_"+fname+".pdf", labels, cross, null);
		LatexTable.printTable(dir+"/cross_month_"+fname+Resident.FR+".tex", "From\\textbackslash To", labels, labels, cross);
		
		double[] aggregate = new double[cross.length];
		for(int i=0; i<aggregate.length;i++)
			for(int j=0; j<cross[i].length;j++)
				aggregate[i] += cross[i][j];
		...
		String title = "From: "+getName(gt_ser_file_from,true)+" To: "+getName(gt_ser_file_to,true);
		title = title.replaceAll("_", " ").trim();
		
		RPlotter.drawBar(GTExtractor.PROFILES,aggregate,"profiles","%",dir+"/cross_month_bar_"+fname+Resident.FR+".pdf", "ggtitle('"+title+"') + theme(plot.title = element_text(size = 16))");
	}
	
	private static String getName(String file, boolean sshort) {
		String name = new File(file).getName().replaceAll(".ser", "");
		name = name.replaceAll("_classes", "");
		if(sshort) {
			name = name.replaceAll("_noregion", "");
		}
		return name;
	}
	...
	/*
	 * This method counts how many users of the classes_ser_file1 can be found in the classes_ser_file2.
	 * It is useful as a cross month measure. Tourists in classes_ser_file of one month should not appear in the classes_ser_file of another month.
	 * Vice versa Residents in the classes_ser_file one month should appear in the classes_ser_file of another month.
	 * 
	...
	public static double[][] corssMonthAnalysis(String classes_ser_file_from, String classes_ser_file_to) throws Exception {
		
		Map<String,String> user_prof_from = (Map<String,String>)CopyAndSerializationUtils.restore(new File(classes_ser_file_from));
		Map<String,String> user_prof_to = (Map<String,String>)CopyAndSerializationUtils.restore(new File(classes_ser_file_to));
		
		double[] denominator = new double[GTExtractor.PROFILES.length];
		for(String profile: user_prof_from.values()) {
			denominator[GTExtractor.P2I.get(profile)]++;
		}
		
		double[][] cross = new double[GTExtractor.PROFILES.length][GTExtractor.PROFILES.length];
		
		
		for(String user: user_prof_from.keySet()) {
			String profile_from = user_prof_from.get(user);
			String profile_to = user_prof_to.get(user);
			if(profile_to != null) 
				cross[GTExtractor.P2I.get(profile_from)][GTExtractor.P2I.get(profile_to)]++;
			else if(profile_from.equals("Resident")) {
				System.out.println(classes_ser_file_from+" == Resident not found ==> "+user);
			}
		}
		...
		for(int i=0; i<cross.length;i++) 
			for(int j=0; j<cross[i].length;j++)
				cross[i][j] /= denominator[i];
		
		
		System.out.println();
		for(int i=0; i<cross.length;i++) {
			for(int j=0; j<cross[i].length;j++)
				System.out.print(F2.format(cross[i][j])+"\t");
			System.out.println();
		}
...
//		runCorssMonthAnalysis(Config.getInstance().base_folder+"/UserEventCounter/file_pls_ve_Venezia_cellXHour_March2014.csv", Config.getInstance().base_folder+"/Tourist/Firenze_March2014_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");
		...
		runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Firenze_July2013_noregion_classes.ser", Config.getInstance().base_folder+"/Tourist/Firenze_March2014_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
		runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Firenze_March2014_noregion_classes.ser", Config.getInstance().base_folder+"/Tourist/Firenze_July2013_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
		
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Venezia_July2013_noregion_classes.ser", Config.getInstance().base_folder+"/Tourist/Venezia_March2014_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Firenze_July2013_noregion_classes.ser", Config.getInstance().base_folder+"/Tourist/Venezia_July2013_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Venezia_July2013_noregion_classes.ser", Config.getInstance().base_folder+"/Tourist/Firenze_July2013_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Lecce_Aug2014_noregion_classes.ser", Config.getInstance().base_folder+"/Tourist/Lecce_Sep2014_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
		
		// FROM GT PROFILES TO GT PROFILES
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Firenze_gt_profiles_July2013.ser", Config.getInstance().base_folder+"/Tourist/Firenze_gt_profiles_March2014.ser",Config.getInstance().paper_folder+"/img/Cross");		
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Firenze_gt_profiles_March2014.ser", Config.getInstance().base_folder+"/Tourist/Firenze_gt_profiles_July2013.ser",Config.getInstance().paper_folder+"/img/Cross");		
				
		// FROM GT PROFILES TO ALL
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Firenze_gt_profiles_July2013.ser", Config.getInstance().base_folder+"/Tourist/Firenze_March2014_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
		//runCorssMonthAnalysis(Config.getInstance().base_folder+"/Tourist/Firenze_gt_profiles_March2014.ser", Config.getInstance().base_folder+"/Tourist/Firenze_July2013_noregion_classes.ser",Config.getInstance().paper_folder+"/img/Cross");		
