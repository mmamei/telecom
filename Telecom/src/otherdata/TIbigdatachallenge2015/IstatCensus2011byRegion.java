...
public class IstatCensus2011byRegion {
	...
	private static IstatCensus2011byRegion instance;
	private Map<String,int[]> data;
	
	
	public static IstatCensus2011byRegion getInstance() {
		if(instance == null) 
			instance = new IstatCensus2011byRegion();
		return instance;
	}
	
	private IstatCensus2011byRegion() {
		try {
			...
			while((line=br.readLine())!=null) {
				...
				if(e[1].indexOf("/")>0)
					e[1] = e[1].substring(0,e[1].indexOf("/"));
				
				e[1] = e[1].replaceAll("/Vallée d'Aoste", "");
				e[1] = e[1].replaceAll("Friuli-Venezia", "Friuli Venezia");
				
				
				String region = e[1].trim().substring(0).toUpperCase();
				
				int[] d = new int[134];
				for(int i=0; i<d.length;i++)
					d[i] = Integer.parseInt(e[i+7]);
				
				int[] v = data.get(region);
				if(v == null) 
					data.put(region, d);
				else
					data.put(region, add(d,v));
			}
			br.close();
		}catch(Exception e) {
			e.printStackTrace();
	...
	public AddMap computeDensity(int var, boolean procapita, boolean print) {
		System.err.println("IstatCensus by Region: "+DIMENSIONS[var]);
		...
		if(print) {
			try {
				KMLHeatMap.drawHeatMap(Config.getInstance().base_folder+"/TIC2015/"+DIMENSIONS[var]+"-byRegion.kml",density,rm,"",!procapita);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	...
	public static void main(String[] args) throws Exception {
		IstatCensus2011byRegion ic = IstatCensus2011byRegion.getInstance();
