...
public class WekaTrainer {
	...
	static String[] classifiers = new String[]{"j48"};//,"1R","logistic","nn1","neural","clustering-kmeans","clustering-som"};
		...
		for(File file: resampledDir.listFiles()) {
			if(file.getAbsolutePath().endsWith(".arff"))
				for(String classifier: classifiers) {
					File willBeOutput = new File(file.getAbsolutePath().replace(".arff", "_"+classifier+".model"));
					if(!willBeOutput.exists() || willBeOutput.lastModified() < file.lastModified())
	...
	public static void train(String arff, String classifier) throws Exception {
		...
		System.out.println("\n************************************************************************************************************");
		System.out.println(classifier+"  "+arff+"\n");
		...
		*/
		...
		else if(classifier.equals("neural")) {
