...
public class GroundTruthAnalysis {
	...
	public static void plotBaseGT() {
		...
		TextPlotter.getInstance().run(tm, "src/analysis/tourist/GroundTruthAnalysisBase.ftl", Config.getInstance().paper_folder+"/img/groundtruth-base.tex");
	...
	public static void plotCorr() throws Exception {
		...
		double[] scaled = new double[res.length];
		for(int i=0; i<scaled.length;i++)
			scaled[i] = sr.predict(res[i]);
		
		
		System.out.println("r = "+sr.getR());
		System.out.println("r^2 = "+sr.getRSquare());
		
		DescriptiveStatistics rel_abs_error = new DescriptiveStatistics();
		
	
		for(int i=0; i<gt.length;i++) 
			rel_abs_error.addValue(Math.abs(gt[i]-scaled[i]) / gt[i]);
		
		System.out.println("median rel_abs_error = "+rel_abs_error.getPercentile(50));
		
		RPlotter.drawScatter(gt, scaled, "GT Tourists", "CDR Tourists", Config.getInstance().paper_folder+"/img/correlation.pdf", "stat_smooth(method=lm,colour='black') + theme(legend.position='none') + geom_point(size = 5)");
		...
		tm.put("med_rel_abs_error", ((int)(100.0*rel_abs_error.getPercentile(50))));
		/*
