...
public class CDRSpaceDensity implements Serializable {
	...
	*/
	...
	private CDRSpaceDensity(String events, RegionMap map, Placemark placemark, Integer max_days) throws Exception {
		...
		List<CDR> actualEvents = new ArrayList<CDR>();
		...
		for(int i=5;i<p.length;i++) {
			try {
				...
				actualEvents.add(new CDR(user_id,mnt,String.valueOf(celllac),String.valueOf(cal.getTimeInMillis())));
	...
	public static void process(RegionMap rm, String cellXHourFile, String gt_ser_file, Integer max, Integer maxdays, String wekaFileName, Placemark placemark) throws Exception {
		...
		while((line=br.readLine())!=null) {
			...
			try {
				td = new CDRSpaceDensity(line,rm,placemark,maxdays);
			} catch(Exception e) {
