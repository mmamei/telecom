...
public class DailyAnalysis {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 * NOTA la visualizzazione di Torino Oct 2014 � sbagliata!!!!!!!!!!!!
		 * I dati vanno da 20 Oct 14 al 14 Nov 14
		 * Quindi particamente i giorni 1-14 sono in verit� giorni di novembre.
		 * Poi c'� un buco. dal 15 al 19
	...
	public static Map<String,double[]> getDailyTrend(String classes_ser_file, String cellXHourFile) throws Exception {
		...
		// smoothing, if v = 0 repla
		for(String p: profile_dailytrend.keySet()) {
			double[] v = profile_dailytrend.get(p);
			for(int i=0; i<v.length;i++) {
				if(v[0] == 0) v[0] = v[1];
				if(v[v.length-1] == 0) v[v.length-1] = v[v.length-2];
				if(i>0 && i<v.length-1 && v[i] < v[i-1]/10 && v[i] < v[i+1]/10) v[i] = (v[i-1]+v[i+1])/2;
			}
			profile_dailytrend.put(p, v);
		}
