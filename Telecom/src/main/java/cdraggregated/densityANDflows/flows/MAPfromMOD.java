...
//	static String fileMOD = "temp/od.csv";
	...
	//	Le province visualizzate nel file html che verr� generato
	static String[] provToVisualize = {"015"};
	
	//	I comuni visualizzati nel file html che verr� generato
...
//	stringa contenente le informazioni sui tragitti "delay", ossia quelli che non rientrano pi� in questa fascia oraria
//	formato str: "da.lat"-"da.lon"-"da.lat"-"da.lon"-"flusso"-"nuovo orario"|
	
	static int countGhostRoute = 0; // non dovrebbe servire
// il  numero di tragitti del quale non si � riuscito a calcolcare il percorso
...
//			System.out.println("velocit� media : "+edges.get(revGeo.getEdge()).getDistance()/edges.get(revGeo.getEdge()).getWeight());
//		}
...
//		if(daIstat)	Write.averageTime2(time, fileMOD, percorsoInput, orario);
		 ...
		 * la chiave � l'id osm (graphopper) che ideintifica un tratto di srtata
