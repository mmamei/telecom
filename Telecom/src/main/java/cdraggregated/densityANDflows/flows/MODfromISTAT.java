...
//	se true il codice generer� 4 file MOD per ogni fascia oraria, ognuno riferito al tempo impiegato a compiere il tragitto,
...
//				if(WriteValue(r,ora,c))				///con questa riga potrei eliminare la funzione "centraConRegione", ma non so il perch� con questa riga il lancio del codice diventa troppo lungo.... 
		...
		for(String j:comuni.keySet()){
			...
			if(!comuni.get(j).getNome().equals("vuoto")){
				...
				if(comuni.get(j).centraConRegione(provinceInConsiderazione)){
					...
					if(comuniOk.get(j).InProvince(provinceInConsiderazione))
